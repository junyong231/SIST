...
public class Ex01_08 {
	...
	public static void main(String[] args) {
		...
		int kor;
		try {
			kor = getScore("국어");//메소드에서 걸린 조건 때문에 얘는 예외가 생길 수도 있는 놈이 됨 -> try/catch해서 해결하거나 또 throw해서 또 넘김 == 처리 안함*여기가 main이라서
	...
	private static int getScore(String 과목명) throws ScoreOutOfBoundException {// 호출한 곳에서 처리하라
		...
		if (input.matches(regex)) {
			...
			//강제로 예외발생시키기
			//사용자 예외 처리 객체
			throw new ScoreOutOfBoundException("점수 범위 (0~100) 벗어남. ");//내가 만든 예외 처리 클래스
		}
	}
